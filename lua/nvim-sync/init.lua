...
M.setup = function(opts)
  ...
  if not user_config or not user_config.username or not user_config.repo_name then
    ...
    vim.notify("Nvim Sync configuration saved! Repository: " .. user_config.username .. "/" .. user_config.repo_name, vim.log.levels.INFO, { title = "Nvim Sync" })
  end
  ...
  sync_module.pull = function()
    ...
    vim.notify("Pulling changes from remote...", vim.log.levels.INFO, { title = "Nvim Sync" })
    run_git_cmd("pull origin " .. config.branch, function(success, code)
      if success then
    ...
    run_git_cmd("add .", function(add_success)
      ...
      run_git_cmd("diff --cached --quiet", function(no_changes)
        ...
        run_git_cmd(commit_cmd, function(commit_success)
          ...
          -- Push to remote
          vim.notify("Pushing changes to remote...", vim.log.levels.INFO, { title = "Nvim Sync" })
          run_git_cmd("push origin " .. config.branch, function(push_success)
  ...
  sync_module.sync = function()
    ...
    run_git_cmd("pull origin " .. config.branch, function(pull_success)
    ...
    local function run_next_command(index)
      ...
      run_git_cmd(init_commands[index], function(success)
        if success then
          ...
          vim.notify(string.format("Failed at step: %s", init_commands[index]), vim.log.levels.ERROR, { title = "Nvim Sync" })
        end
      end)
    end
  ...
  sync_module.configure = function()
    ...
    local remote_url = string.format("https://github.com/%s/%s.git", username, repo_name)
    run_git_cmd(string.format('remote set-url origin %s', remote_url), function(success)
      if success then
        vim.notify(string.format("Repository updated to %s/%s", username, repo_name), vim.log.levels.INFO, { title = "Nvim Sync" })
      else
  ...
  local function setup_repo_on_startup()
    vim.schedule(function()
      ...
      if not is_git_repo() then
          ...
          if init_success then
            run_git_cmd(string.format('remote add origin %s', remote_url), function(remote_success)
              if remote_success then
                vim.notify(string.format("Repository %s/%s configured", username, repo_name), vim.log.levels.INFO, { title = "Nvim Sync" })
                -- Start sync after setup
                vim.defer_fn(function() sync_module.sync() end, 1000)
              end
            end)
          end
        end)
        ...
        run_git_cmd(string.format('remote set-url origin %s', remote_url), function(success)
          if success then
            vim.notify(string.format("Using repository %s/%s", username, repo_name), vim.log.levels.INFO, { title = "Nvim Sync" })
            -- Start sync after setup
            vim.defer_fn(function() sync_module.sync() end, 1000)
          end
        end)
